...
For an introduction to Graph Machine Learning, we refer the interested reader to the  [Stanford CS224W: Machine Learning with Graphs](https://www.youtube.com/watch?v=JAB_plj2rbA) lectures.
For an interactive introduction to PyG, we recommend our carefully curated  [Google Colab](colabs.html) notebooks.
...
A single graph in PyG is described by an instance of [torch_geometric.data.Data](../generated/torch_geometric.data.Data.html#torch_geometric.data.Data), which holds the following attributes by default:
...
In fact, the [Data](../generated/torch_geometric.data.Data.html#torch_geometric.data.Data) object is not even restricted to these attributes.
...
> **Note:** PyTorch and `torchvision` define an example as a tuple of an image and a target.
...
This is needed as we want our final data representation to be as compact as possible, *e.g.*, we want to index the source and destination node features of the first edge `(0, 1)` via `x[0]` and `x[1]`, respectively.
You can always check that your final [Data](../generated/torch_geometric.data.Data.html#torch_geometric.data.Data) objects fulfill these requirements by running [validate()](../generated/torch_geometric.data.Data.html#torch_geometric.data.Data.validate):
...
Besides holding a number of node-level, edge-level or graph-level attributes, [Data](../generated/torch_geometric.data.Data.html#torch_geometric.data.Data) provides a number of useful utility functions, *e.g.*:
...
You can find a complete list of all methods at [torch_geometric.data.Data](../generated/torch_geometric.data.Data.html#torch_geometric.data.Data).
...
An initialization of a dataset will automatically download its raw files and process them to the previously described [Data](../generated/torch_geometric.data.Data.html#torch_geometric.data.Data) format.
...
This time, the [Data](../generated/torch_geometric.data.Data.html#torch_geometric.data.Data) objects holds a label for each node, and additional node-level attributes: `train_mask`, `val_mask` and `test_mask`, where
...
PyG contains its own [torch_geometric.loader.DataLoader](../modules/loader.html#torch_geometric.loader.DataLoader), which already takes care of this concatenation process.
...
[torch_geometric.data.Batch](../generated/torch_geometric.data.Batch.html#torch_geometric.data.Batch) inherits from [torch_geometric.data.Data](../generated/torch_geometric.data.Data.html#torch_geometric.data.Data) and contains an additional attribute called `batch`.
...
You can learn more about the internal batching procedure of PyG, *e.g.*, how to modify its behavior, [here](../advanced/batching.html).
For documentation of scatter operations, we refer the interested reader to the `torch_scatter` [documentation](https://pytorch-scatter.readthedocs.io).
...
PyG comes with its own transforms, which expect a [Data](../generated/torch_geometric.data.Data.html#torch_geometric.data.Data) object as input and return a new transformed [Data](../generated/torch_geometric.data.Data.html#torch_geometric.data.Data) object.
Transforms can be chained together using [torch_geometric.transforms.Compose](../generated/torch_geometric.transforms.Compose.html#torch_geometric.transforms.Compose) and are applied before saving a processed dataset on disk (`pre_transform`) or before accessing a graph in a dataset (`transform`).
...
If the `pre_transform` does not match with the one from the already processed dataset, you will be given a warning.


In addition, we can use the `transform` argument to randomly augment a [Data](../generated/torch_geometric.data.Data.html#torch_geometric.data.Data) object, *e.g.*, translating each node position by a small number:
...
The constructor defines two [GCNConv](../generated/torch_geometric.nn.conv.GCNConv.html#torch_geometric.nn.conv.GCNConv) layers which get called in the forward pass of our network.
Note that the non-linearity is not integrated in the `conv` calls and hence needs to be applied afterwards (something which is consistent across all operators in PyG).
...
The easiest way to learn more about Graph Neural Networks is to study the examples in the `examples/` directory and to browse `torch_geometric.nn`.
Happy hacking!
...
2. Load the `"IMDB-BINARY"` dataset from the [TUDataset](../generated/torch_geometric.datasets.TUDataset.html#torch_geometric.datasets.TUDataset) benchmark suite and randomly split it into 80%/10%/10% training, validation and test graphs.
