One of the design principles of PyG is that models and data loading routines should be exchangeable to allow for flexible GNN and data loading experimentation.
As such, models can usually be written in a data loading agnostic fashion, independent of whether one applies full-batch or mini-batch training strategies via, *e.g.*, [DataLoader](../modules/loader.html#torch_geometric.loader.DataLoader), [NeighborLoader](../modules/loader.html#torch_geometric.loader.NeighborLoader) or [ClusterLoader](../modules/loader.html#torch_geometric.loader.ClusterLoader).
...
One such limitation is that a GNN trained with the [NeighborLoader](../modules/loader.html#torch_geometric.loader.NeighborLoader) routine iteratively builds representations for *all* nodes at *all* depths of the network, although nodes sampled in later hops do not contribute to the node representations of seed nodes in later GNN layers anymore, thus performing useless computation.


*Hierarchical Neighborhood Sampling* or *Hierarchical Graph Adjacency Matrix (HGAM)* is a technique available in PyG to eliminate this overhead and speeds up training and inference in mini-batch GNNs.
...
To support this trimming and implement it effectively, the [NeighborLoader](../modules/loader.html#torch_geometric.loader.NeighborLoader) implementation in PyG and in pyg-lib additionally return the number of nodes and edges sampled in hop.
This information allows for fast manipulation of the adjacency matrix, which in turns lead to great computation reduction.
The [NeighborLoader](../modules/loader.html#torch_geometric.loader.NeighborLoader) prepares this metadata via the dedicated attributes `num_sampled_nodes` and `num_sampled_edges`.
It can be accessed from the [Batch](../generated/torch_geometric.data.Batch.html#torch_geometric.data.Batch) object returned for both homogeneous and heterogeneous graphs.


To sum up, HGAM is special data structure that enables efficient message passing computation in [NeighborLoader](../modules/loader.html#torch_geometric.loader.NeighborLoader) scenarios.
HGAM is implemented in PyG and can be utilized via the special [trim_to_layer()](../modules/utils.html#torch_geometric.utils.trim_to_layer) functionality.
HGAM is currently an option that PyG users are free to switch on, or leave it off *(current default)*.


## Usage


Here, we show examples of how to use the HGAM functionality in combination with [NeighborLoader](../modules/loader.html#torch_geometric.loader.NeighborLoader):
...
The attributes `num_sampled_nodes` and `num_sampled_edges` can be used by the [trim_to_layer()](../modules/utils.html#torch_geometric.utils.trim_to_layer) function inside the GNN:
...
- `examples/hierarchical_sampling.py`: An [example](https://github.com/pyg-team/pytorch_geometric/blob/master/examples/hierarchical_sampling.py) to show-case the basic usage of HGAM.
- `examples/hetero/hierarchical_sage.py`: An [example](https://github.com/pyg-team/pytorch_geometric/blob/master/examples/hetero/hierarchical_sage.py) of HGAM on heterogeneous graphs.
