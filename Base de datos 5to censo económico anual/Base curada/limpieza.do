...
drop CCPP PROVINCIA CCDI DISTRITO ZONA_ORD MANZANA_ORD FRENTE_ORD
...
// CIIU 3 dígitos: Grupo (agregación sectorial media)
gen ciiu_3dig = substr(C3P4_COD, 1, 3) if C3P4_COD != "" & ciiu_length >= 3
label variable ciiu_3dig "CIIU Grupo (3 dígitos)"
...
// CIIU 6 dígitos: Subclase (máxima desagregación sectorial)
gen ciiu_6dig = substr(C3P4_COD, 1, 6) if C3P4_COD != "" & ciiu_length >= 6
label variable ciiu_6dig "CIIU Subclase (6 dígitos)"

// Diagnóstico: Verificar distribución de empresas por nivel de agregación
tab ciiu_2dig, missing
tab ciiu_3dig if !missing(ciiu_3dig)
tab ciiu_4dig if !missing(ciiu_4dig)
tab ciiu_6dig if !missing(ciiu_6dig)
...
quietly distinct ciiu_3dig
display "Clusters CIIU 3 dígitos: " r(ndistinct)
quietly distinct ciiu_4dig
display "Clusters CIIU 4 dígitos: " r(ndistinct)
quietly distinct ciiu_6dig
display "Clusters CIIU 6 dígitos: " r(ndistinct)
