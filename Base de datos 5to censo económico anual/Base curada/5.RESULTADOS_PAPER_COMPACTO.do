...
// Estandarizar ventas (necesario para convergencia)
summarize ventas_soles_2021
gen ventas_std = (ventas_soles_2021 - r(mean)) / r(sd)
label var ventas_std "Ventas 2021 estandarizadas (media=0, sd=1)"
...
tabstat productividad_x_trabajador ventas_soles_2021, by(regimen) stat(mean n)
...
logit op2021_original c.ruc##(i.region) ventas_std i.sector i.sexo_gerente productividad_k digital_score tributos_k salarios_k i.tipo_local i.regimen, vce(cluster ciiu_4dig)
...
quietly regress op2021_original productividad_k tributos_k salarios_k digital_score i.ventas_std sexo_gerente i.region i.sector i.regimen i.tipo_local
vif

/*=============================================================================
4. ESPECIFICACIONES ALTERNATIVAS: TABLA 3 DEL PAPER (Sección 5.1)
=============================================================================*/

display ""
display "══════════════════════════════════════════════════════════"
display "TABLA 3: ROBUSTEZ - ESPECIFICACIONES ALTERNATIVAS"
display "══════════════════════════════════════════════════════════"
display ""

// (2) Sin tributos/remuneraciones
logit op2021_original c.ruc##(i.region) ventas_std i.sector sexo_gerente ///
      productividad_k digital_score i.tipo_local, vce(cluster ciiu_4dig)
estimates store sin_tributos_salarios

// (3) Solo controles exógenos
logit op2021_original c.ruc##(i.region) i.sector sexo_gerente ///
      i.tipo_local, vce(cluster ciiu_4dig)
estimates store solo_exogenos

// (4) Solo microempresas
preserve
keep if ventas_soles_2021 < 50000 & !missing(ventas_soles_2021)
display "Observaciones en submuestra microempresas: " _N
...
logit op2021_original c.ruc##(i.region) ventas_std_micro i.sector sexo_gerente ///
      productividad_k digital_score i.tipo_local, vce(cluster ciiu_4dig)
estimates store solo_micro
restore

// (5) Solo pequeñas empresas
preserve
keep if ventas_soles_2021 >= 50000 & !missing(ventas_soles_2021)
display "Observaciones en submuestra pequeñas empresas: " _N

// Re-estandarizar ventas en submuestra
quietly summarize ventas_soles_2021
gen ventas_std_pequena = (ventas_soles_2021 - r(mean)) / r(sd)

logit op2021_original c.ruc##(i.region) ventas_std_pequena i.sector sexo_gerente ///
      productividad_k digital_score i.tipo_local, vce(cluster ciiu_4dig)
...
marginsplot, title("Efecto de RUC según escala de ventas") ///
            ytitle("Efecto marginal de RUC") xtitle("Ventas estandarizadas (SD)")

display ""
display "══════════════════════════════════════════════════════════"
display "EFECTOS POR PRODUCTIVIDAD (Sección 3.2)"
display "══════════════════════════════════════════════════════════"
display ""

// Probabilidades predichas por productividad
margins region, at(productividad_k = (0(10)120))
marginsplot, title("Supervivencia según productividad laboral") ///
            ytitle("Pr(Supervivencia)") xtitle("Productividad (miles S/./trabajador)")
...
/*
/*=============================================================================
9. ESTADÍSTICAS RESUMEN PARA TEXTO DEL PAPER
=============================================================================*/

display ""
display "══════════════════════════════════════════════════════════"
display "ESTADÍSTICAS CLAVE PARA TEXTO DEL PAPER"
display "══════════════════════════════════════════════════════════"
display ""

// Composición de la muestra
display "--- COMPOSICIÓN DE LA MUESTRA (Sección 1.1) ---"
tab ruc
tab region
tab sector
...
display ""
display "--- DISTRIBUCIÓN POR RÉGIMEN TRIBUTARIO (Sección 1.3) ---"
tab regimen

// Supervivencia por régimen
display ""
display "--- TASAS DE SUPERVIVENCIA POR RÉGIMEN (Sección 4.2) ---"
tab op2021_original regimen, col nofreq
...
display "  1. Tabla1_ModeloPrincipal.xls"
display "  2. Tabla2_Robustez.xls"
display "  3. Tabla3_EfectosMarginales.xls"
display "  4. Log file: resultados_paper_$(S_DATE).log"
display ""
display "PRÓXIMOS PASOS:"
display "  - Revisar tablas en Word/Excel"
display "  - Copiar estadísticas clave al documento 7. RESULTADOS.md"
display "  - Preparar gráficos de efectos marginales para presentación"
display "══════════════════════════════════════════════════════════"
