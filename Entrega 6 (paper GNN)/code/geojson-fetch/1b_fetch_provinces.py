...
TEMP_DIR = os.path.join(OUTPUT_DIR, "temp_provinces")
FINAL_OUTPUT = os.path.join(OUTPUT_DIR, "peru_provinces.geojson")
...
def process_department(dept):
    ...
    # admin_level=6 is Provinces
    query = f'[out:json][timeout:{timeout}]; area({area_id})->.a; rel(area.a)["admin_level"="6"]; out body geom;'
    
    print(f"  [{dept_name}] Fetching provinces...")
    ...
    print(f"  [{dept_name}] Saved {len(features)} provinces")
    return features
...
def main():
    ...
    print(f"\n[Done] {len(all_features)} provinces saved to {FINAL_OUTPUT}")
